...
export default class ClangTidyLinter {
  ...
  lint (editor) {
    ...
    return this.clangTidy.check(extraArgs, file).then(result => {
      return new Promise((resolve, reject) => {
        ...
        const linterMessages = result.map(message => convertToLinterMessage(message))
        this.fixes.set(
          file,
          linterMessages.map(m => {
            return {
              range: m.range,
              fixes: m.fixes
            }
  ...
  getFixes (file, point) {
    if (!this.fixes) return undefined

    if (this.fixes.has(file)) {
      const allFixes = this.fixes.get(file) || []
      return allFixes.find(elem => elem.range.containsPoint(point))
    } else {
      return undefined
    }
  }
