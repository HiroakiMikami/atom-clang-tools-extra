...
export default {
  ...
  activate (state) {
    ...
    this.provider = new ClangTidyAutocompleteProvider(this.clangTidy)
    this.provider.selector = this.grammars.map(elem => `.${elem}`).join(','),
    this.provider.inclusionPriority = 1
